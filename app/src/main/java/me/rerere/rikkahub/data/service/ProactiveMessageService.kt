...
class ProactiveMessageTriggerService : android.app.Service(), KoinComponent {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                val tools = buildTools(settings, assistant, conversationId, conversation)
    ...
    private suspend fun buildTools(
        ...
        return buildList {
            // 本地工具（绑定正确的 ToolInvocationContext）
            addAll(
                localTools.getTools(
            ...
            // 搜索工具（正确调用 createSearchTools(settings)）
            if (settings.enableWebSearch) {
                addAll(createSearchTools(settings))
            }

            // 工作区沙盒工具（正确检查 assistant.workspaceId 状态）
            val workspaceId = assistant.workspaceId?.toString()
            if (!workspaceId.isNullOrBlank()) {
                val workspace = workspaceRepository.getById(workspaceId)
                if (workspace?.shellStatus == WorkspaceShellStatus.READY.name) {
                    addAll(createWorkspaceTools(workspaceId, workspaceRepository, conversation?.workspaceCwd))
                }
            }
